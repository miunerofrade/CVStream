...
def execute_video_task(page, target_url, asr_worker, export_base_dir, stop_event, target_date=None, need_subtitle=True, keep_media=False):
    ...
    for item in target_items:
        ...
        need_media = keep_media or (need_subtitle and not got_official_sub)
        audio_only_mode = (not keep_media) and (need_subtitle and not got_official_sub)
            ...
            if final_url:
                ...
                try:
                    ...
                    if need_subtitle and not got_official_sub:
                        yield f"[{get_time()}] 启动云端 ASR 音频转写..."
