...
def load_config():
    ...
    if os.path.exists(CONFIG_FILE):
        with open(CONFIG_FILE, "r", encoding="utf-8") as f:
            ...
            defaults = {
                ...
                "need_subtitle": True,
                "keep_media": False,
                "asr_engine": "阿里 Paraformer (DashScope)",
    ...
    return {
        ...
        "headless_mode": False, "need_subtitle": True, "keep_media": False,
        "asr_engine": "阿里 Paraformer (DashScope)", "llm_engine": "DeepSeek (api.deepseek.com)",
...
def select_folder():
    ...
    folder_path = filedialog.askdirectory(master=root, title="选择导出文件夹")
    root.destroy()
    return folder_path
...
def _launch_browser(playwright, headless, args):
    """按平台优先选用原生浏览器，逐级回退到 Playwright 内置引擎。
    Windows  → Edge → Chrome → 内置 Chromium
    macOS    → Safari(WebKit) → Chrome → 内置 Chromium
    Linux    → Firefox → Chrome → 内置 Chromium
    """
    import platform
    system = platform.system()

    # macOS: 优先 Safari (WebKit 引擎，系统自带)
    if system == "Darwin":
        try:
    ...
    # Windows / 通用回退: 尝试已安装的 Chromium 系浏览器
    for channel in ("msedge", "chrome"):
        try:
            return playwright.chromium.launch(headless=headless, args=args, channel=channel)
        except Exception:
            continue

    # 最终兜底: Playwright 自带的 Chromium
...
def run_manual_init_login(target_url, username, password):
    ...
    try:
        with sync_playwright() as p:
            ...
            browser = _launch_browser(p, headless=False, args=browser_args)
...
def run_fetch_dates_pipeline(target_url, username, password, headless):
    ...
    try:
        with sync_playwright() as p:
            ...
            browser = _launch_browser(p, headless=actual_headless, args=browser_args)
...
def run_pipeline(target_url, username, password, headless, asr_worker, stop_event, need_subtitle, keep_media, target_date=None):
    def get_time(): return time.strftime('%H:%M:%S')
...
def init_session_state(config):
    ...
    default_settings = {
        ...
        "asr_engine": config.get("asr_engine", "阿里 Paraformer (DashScope)"),
...
st.set_page_config(layout="wide", page_title="CVStream Lite")
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("<div style='padding: 30px 0 20px 15px; font-size: 18px; font-weight: 400; letter-spacing: 3px; color: #111;'>CVSTREAM LITE</div>", unsafe_allow_html=True)
    ...
    if selected == "参数配置":
        ...
        with date_col1:
            # 「全部日期」仅在仅开启字幕抓取时可用
            subtitle_only = need_subtitle and not keep_media
            date_options = ["自动获取最新"]
            if subtitle_only:
        ...
        with st.container(border=True):
            ...
            if is_adding_new_asr:
                ...
                with add_c1:
                    new_asr_name = st.text_input("端点名称", placeholder="例如：我的私有 Whisper API", key="new_asr_name")
                with add_c2:
                    new_asr_url = st.text_input("API URL", placeholder="wss://...", key="new_asr_url")
                with add_c3:
                ...
                with c_url:
                    st.text_input("ASR URL", value=ALL_ASR_ENDPOINTS.get(selected_asr, ""), disabled=True, key="asr_url_display")

                with c_key:
                    saved_asr_key = config.get("asr_api_key", "")

                    is_locked = bool(saved_asr_key)

                    if is_locked:
                        st.text_input("API 密钥", value="*" * 20, disabled=True, key="asr_key_locked")
                    else:
            ...
            if run_btn:
                if not target_url or not target_url.startswith("http"):
                    ...
                    if not current_asr_version or current_asr_version == "+ 添加自定义模型版本...":
                        st.error("拦截操作：ASR 引擎未指定具体的模型版本！请先在上方选择。")
                        st.stop()
                    if not config.get("asr_api_key", "").strip():
                        st.error("拦截操作：未配置 ASR 的 API 密钥 (Key)！请在上方填写并保存。")
                        st.stop()
                    ...
                    with st.status("正在执行任务...", expanded=True) as status:
                        ...
                        st.rerun()
        st.divider()
        st.caption("CVStream Lite © 2026 - 保留所有权利")
